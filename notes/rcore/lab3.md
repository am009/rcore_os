...
内核的地址空间要抬高, 不是平移256G. 数据段起始地址变成
0xffff ffff 8020 0000, 而原来是
0x0000 0000 8020 0000
多了0xffff 0000 0000, 也就是平移了508G.
...
由于内核换了位置(使用了虚拟地址), 需要在memory/config中加入MMIO 设备段内存区域起始地址: DEVICE_START_ADDRESS, 和DEVICE_END_ADDRESS, 另外还要将kernel_end_address 改成虚拟地址, config里的高位地址都要改改.
(MMIO表示memory mapped io. 访问这里的地址就是直接与外设交互)


## 其他问题

启动时候的页表, 这里Access 和Dirty没必要吧?? 有什么好处??
